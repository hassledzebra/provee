// JavaScript Document
function CP2tform(warp,unwarp){
	// warp is the coordinates of control points of warped image; unwarp is the coordinates of control points of unwarped image;
	
	var n = unwarp.length; // the number of control points pair
	var Dmatrix=zeros(n,3);  // deformation matrix
	var XWmatrix = zeros(n,1);
	var YWmatrix = zeros(n,1);
	
	//console.log(XWmatrix);
	for(var i=0;i<n;i++){
		// I only need 3 control point pairs, this is different from what I have learnt;
		Dmatrix[i][0] = unwarp[i][0];
		Dmatrix[i][1] = unwarp[i][1];
		Dmatrix[i][2] = 1; 
		XWmatrix[i][0] = warp[i][0];
		YWmatrix[i][0] = warp[i][1];
	}
	var Drev = pseudoInverse(Dmatrix);
	//console.log(XWmatrix);
	var Amatrix = zeros(3,1);
	var Bmatrix = zeros(3,1);
	
	Amatrix = matrixMultiply(Drev,XWmatrix);
	Bmatrix = matrixMultiply(Drev,YWmatrix);
	
	
	var output = [[0,0,0],[0,0,0],[0,0,1]];
	
	output[0][0] = Amatrix[0][0];
	output[0][1] = Amatrix[1][0];
	output[0][2] = Amatrix[2][0];
	
	output[1][0] = Bmatrix[0][0];
	output[1][1] = Bmatrix[1][0];
	output[1][2] = Bmatrix[2][0];

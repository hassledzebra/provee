...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		fileTree: function(options, file) {
			...
			if( options.folderEvent2	=== undefined ) options.folderEvent2	= 'mouseenter';
			if( options.folderEvent3	=== undefined ) options.folderEvent3	= 'mouseleave';
					...
					$(element).find('LI A').on(options.folderEvent, function() {
						...
						if( $(this).parent().hasClass('directory') ) {
							if( $(this).parent().hasClass('collapsed') ) {
								...
								$(this).parent().removeClass('collapsed').addClass('expanded'); // this line was missing in the original post
							} else {
								// Collapse
								...
								//$(this).trigger('filetreecollapsed', data);
							...
							file($(this).attr('rel'));
							//$(this).attr('rel');
							//$(this).trigger('filetreeclicked', data);
							//_trigger($(this), 'filetreeclicked', data);
						}
						return false;
					});
					//folderevent2
				$(element).find('LI A').on(options.folderEvent2, function() {
						// set up data object to send back via trigger
						var data = {};
						data.li = $(this).closest('li');
						data.type = ( data.li.hasClass('directory') ? 'directory' : 'file' );
					...
					//folderevent3
				/*$(element).find('LI A').on(options.folderEvent3, function() {
					...
					// Prevent A from triggering the # on non-click events
					if( options.folderEvent2.toLowerCase != 'mouseenter' ) $(element).find('LI A').on('mouseenter', function() { return false; });
					if( options.folderEvent3.toLowerCase != 'mouseleave' ) $(element).find('LI A').on('mouseleave', function() { return false; });
				}
				// Loading message
